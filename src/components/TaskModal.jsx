...
import { ArrowUturnLeftIcon, CheckIcon, TrashIcon, PencilIcon } from '@heroicons/react/24/outline';
...
const getHistoryIcon = (type) => {
  switch (type) {
    ...
    case 'due_date':
      return <XMarkIcon className="w-4 h-4 text-purple-500" />;
    default:
    ...
    const loadHistory = async () => {
      ...
      try {
        if (useFirebase && user) {
          ...
          const q = query(historyRef, where('ticketData.id', '==', task.id), orderBy('timestamp', 'desc'));
  ...
  return (
      ...
      >
        ...
        >
          <div
              ref={modalRef}
              className="bg-white dark:bg-dark-surface-2 rounded-xl shadow-xl w-full max-w-4xl max-h-[95vh] flex flex-col tour-allow-interaction"
              style={{ height: '90vh' }}
          >
            {/* Header */}
